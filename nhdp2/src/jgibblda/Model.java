...
//	public double[] alphaq; //alpha for each cluster
	...
	public void setDefaultValues(){
		wordMapFile = "wordmap2.txt";
		trainlogFile = "trainlog2.txt";
		tassignSuffix = ".tassign2";
		thetaSuffix = ".theta2";
		phiSuffix = ".phi2";
		sigmaSuffix = ".sigma2";
		rhoSuffix = ".rho2";
		othersSuffix = ".others2";
		twordsSuffix = ".twords2";
		mapSuffix = ".mapTopic2.html";
		mapRegionSuffix = ".mapRegion2.html";
...
//			}
		...
		for (int k=0; k<K; k++) {
			...
			for (int i=0;i<triangles.length;i++) {		
				...
				if (sigma[triangles[i][0]][k] > avgP && sigma[triangles[i][1]][k] > avgP && sigma[triangles[i][2]][k] > avgP ) {
					...
					//opacity = (sigma[triangles[i][0]][k] + sigma[triangles[i][1]][k] + sigma[triangles[i][2]][k]) / 3.0;
					...
					//					System.out.println();
					//					System.out.println(lat[0] + " " +lon[2] + " " +triangles[i][0]);
					//					System.out.println(lat[1] + " " +lon[2]+ " " +triangles[i][1]);
					//					System.out.println(lat[2] + " " +lon[2]+ " " +triangles[i][2]);
	...
	public boolean initNewModel(Option option){
		...
		KMeans kmeans = new KMeans(data.docs, J, dir,trainingSize);
//		Cluster1 kmeans = new Cluster1(data.docs, J, dir);

		ArrayList<CopyOnWriteArraySet<Integer>> qd2 = kmeans.qd;		
		qd = new int[qd2.size()][];
