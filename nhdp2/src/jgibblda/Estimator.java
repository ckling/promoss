...
//				.estimateAlphaMap(trnModel.nkt, trnModel.nk, trnModel.beta, trnModel.betaa, trnModel.betab);
	...
	//sample cluster memberships, input determines if we sample for train or test
	public void setCluster (boolean test) {
					
		countj = new int[trnModel.J];

		for (int j = 0;j<trnModel.J;j++) {
			int[] neighbours= trnModel.qqN[j];
			// neighbours plus base cluster
			int[] neighboursBase = new int [neighbours.length+1];
			neighboursBase[0]=j;
			System.arraycopy(neighbours, 0, neighboursBase, 1, neighbours.length);
			int[] docs = trnModel.qd[j];
			for (int doc:docs) {
				//only for training docs or for test docs
				if ((!test && doc < trnModel.trainingSize*trnModel.M) 
						|| (test && doc >= trnModel.trainingSize*trnModel.M)) {
					double[] pCluster = new double[neighboursBase.length];
					//get probability for belonging to cluster or neighbour
					for (int i=0;i<neighboursBase.length;i++) {
						int cluster = neighboursBase[i];
						for (int k=0;k<trnModel.K;k++) {
							pCluster[i] +=  
									mdk[doc][k] * Math.log(trnModel.sigma[cluster][k]);
						}
						pCluster[i] = Math.exp(pCluster[i]);
					}

					//cummulate
					for (int cluster=1;cluster<pCluster.length;cluster++) {
						pCluster[cluster]+=pCluster[cluster-1];
					}
					
					double randSum = Math.random()*pCluster[pCluster.length-1];
					int chosen = 0;
					while(randSum>pCluster[chosen]) {
						chosen++;
					}
					int cluster = neighboursBase[chosen];
					clusterm[doc]=cluster;
					countj[cluster]++;
				}
			}
		}
