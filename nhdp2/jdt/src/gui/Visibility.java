...
public class Visibility {
	 ...
	 *         triangulation)
	 */
	public boolean los(Point_dt p1, Point_dt p2) {
		boolean ans = true;
		if (_dt == null)
		...
		if (!_dt.contains(p1.x(), p1.y()))
			throw new RuntimeException("** ERR: p1:" + p1
					+ " is NOT contained in the triangulation **");
		if (!_dt.contains(p2.x(), p2.y()))
			throw new RuntimeException("** ERR: p2:" + p2
					+ " is NOT contained in the triangulation **");
	...
	void printSection(Point_dt p1, Point_dt p2) {
		...
		for (int i = 0; i < _section.size(); i++) {
			...
			System.out.println(i + ") dist1:" + curr.distance(p1) + "  dist2:"
	...
	void computeSection(Point_dt p1, Point_dt p2) {
	...
	boolean cut(Point_dt pp1, Point_dt pp2, Triangle_dt t) {
		boolean ans = false;
		if (t.isHalfplane())
			return false;
