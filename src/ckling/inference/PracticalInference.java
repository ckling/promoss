...
public class PracticalInference {
	...
	public void run () {
		for (int i=0;i<RUNS;i++) {
			...
			System.out.println(directory + " run " + i + " (alpha_0 "+alpha_0+" alpha_1 "+ alpha_1+ " beta_0 " + beta_0 + " gamma "+gamma + " delta " + delta[0]+ " epsilon " + epsilon[0]);
	...
	public void save () {
		...
		save.close();
		save.saveVar(pi_0, directory+save_prefix+"pi0_"+rhot_step);
		save.close();
		save.saveVar(sumqf, directory+save_prefix+"sumqf_"+rhot_step);
		save.close();
		save.saveVar(alpha_0, directory+save_prefix+"alpha_0_"+rhot_step);
		save.close();
		save.saveVar(alpha_1, directory+save_prefix+"alpha_1_"+rhot_step);
		save.close();

		//We save the large document-topic file every 10 save steps, together with the perplexity
		if ((rhot_step % (SAVE_STEP *10)) == 0) {

			save.saveVar(perplexity(), directory+save_prefix+"perplexity_"+rhot_step);
		...
		for (int f=0; f<F;f++) {
			...
			save.saveVar(feature_cluster_topics[f], directory+save_prefix+"clusters_"+f+"_"+rhot_step);
			save.close();
		}
		...
		save.saveVar(topktopics, directory+save_prefix+"topktopics_"+rhot_step);
		save.saveVar(delta, directory+save_prefix+"delta_"+rhot_step);
		save.saveVar(epsilon, directory+save_prefix+"epsilon_"+rhot_step);
		...
		save.saveVar("alpha_0 "+alpha_0+
				...
				,directory+save_prefix+"others_"+rhot_step);
		...
		save.saveVar(sumfck, directory+save_prefix+"sumfck_"+rhot_step);
		...
		save.saveVar(topic_ge0, directory+save_prefix+"topic_ge0_"+rhot_step);
