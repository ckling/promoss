				...
				for (int k=0;k<K;k++) {
					...
					if (rhot_step==1) {
						...
						q[k] = 	//probability of topic given feature & group
								(nmk1[g][d][k] + mmk[g][d][k] + alpha0[g]*pi0gk[g][k])
	...
	public void updateHyperParameters() {
		...
		if(rhot_step>BURNIN_DOCUMENTS && (rhot_step-1)%1 == 0) {
			...
			for (int g=0;g<c.G;g++) {
				...
				for (int d=0;d<c.Gd[g];d++) {
					eta += Gamma.digamma(alpha0[g])/Gamma.digamma(alpha0[g]+nm[d]);
				}
				
				double summ = BasicMath.sum(mgk[g]);
				
				double[] gapk = new double[K];
				for (int k=0;k<K;k++) {
					gapk[k]=Math.exp(Gamma.digamma(a0 + summ)) / (b0 - eta);
					//pik ~ Dir(m1+1,...,mK+1)
					gapk[k]*=Math.exp(Gamma.digamma(1 + mgk[g][k])) / Math.exp(Gamma.digamma(1 + summ));
				}
				...
				System.out.println("alpha0["+g+"] = " +alpha0[g]);
