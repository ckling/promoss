...
public class DCTM5_CVB {
	...
	public double[] perplexity() {
		...
		String output_base_folder = c.directory + "output_DCTM5/";
	...
	public void inferenceDoc(int m) {
		...
		//Comment inference
			...
			double[][] prior_parts = new double[K][K+1];
			for (int k2=0;k2<K;k2++) {
				for (int k=0;k<K;k++) {
		...
		if(rhot_step>BURNIN_DOCUMENTS && (rhot_step-1)%1 == 0) {
			...
			for (int g=0;g<c.G;g++) {
				...
				if (mg[g] > 0) {
					...
					galpha = Math.exp(Gamma.digamma(mg[g] + a))/(b-eta_sum[g]);
				}
				else {
					galpha = 0;
		...
		//Comment inference
			...
			for (int i=0;i<termIDs.length;i++) {
				...
				for (int k=0;k<K;k++) {	
					...
					double prior = Galphadeltag[g][0] * Gtheta[k] * Math.exp(Gamma.digamma(mgkk[g][k][k] + gamma) - denom);
