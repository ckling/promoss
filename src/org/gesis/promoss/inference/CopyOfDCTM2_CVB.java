	...
	public void inferenceDoc(int m) {
		...
		//Comment inference
			...
			for (int i=0;i<termIDs.length;i++) {
				...
				for (int k2=0;k2<K2;k2++) {				
					q[K*K2+k2] = 
							(nmk2[g][d][ci-1][K*K2+k2] + Galphadeltag[g][k2+1]) * topic_probability[k2]
				...
				if (Double.isNaN(BasicMath.sum(q))) {
					...
					BasicMath.print(topic_probability);
					BasicMath.print(q);
					System.out.println(nmk2[g][d][ci-1][K*K2+0] + " "+ Galphadeltag[g][0+1]);
					//System.out.println(g + " " + d + " " + ci+ " " + k + " " + k2 +  " " +  alpha1[g] + " " +  theta[k] + " " + (mgkk[g][k][k2] +1) + " " +  denom + " " + nmk2[g][d][ci-1][k*K2+k2]);
					//q[k*K2+k2]=0;
				...
				for (int k=0;k<K+1;k++) {	
					for (int k2=0;k2<K2;k2++) {		
						...
						nmk2var[g][d][ci-1][K*K2+k2]+=termfreq*q[l]*(1.0-q[l]);
					}
				}		
	...
	public void updateHyperParameters() {
		...
		if(rhot_step>BURNIN_DOCUMENTS && (rhot_step-1)%1 == 0) {
			...
			for (int g=0;g<c.G;g++) {
				...
				if (mg[g] > 0) {
					alpha = mg[g]/(-eta_sum[g]);
					System.out.println("alpha' " + alpha);
					if (eta_sum[g]>0) {
					galpha = Math.exp(Gamma.digamma(mg[g]))/-eta_sum[g];
				}
				else {
					galpha=alpha;
				}
